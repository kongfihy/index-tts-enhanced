...
[上游项目](https://github.com/index-tts/index-tts) · [许可证](LICENSE)
</div>
...
当前模型推理仍然是**单 Worker 串行执行**。多进程推理尚未作为默认功能开放，后续会在完成稳定性、吞吐和内存验证后再评估。
...
公开版本会优先继续完善多候选生成、候选结果试听与选择，以及不同硬件环境下的推理效率。并发推理仍需经过充分的稳定性、吞吐、可复现性和内存验证后再决定是否开放。

具体执行记录和本地测试计划不随公开仓库同步。
